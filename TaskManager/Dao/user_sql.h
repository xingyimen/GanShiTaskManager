...
public:
    ...
    //连接数据库
    bool connect_mysql(QSqlDatabase &database);
    //注册
    ...
    void write_ScheduleDao(int user_id,ScheduleDao item,QSqlDatabase database);
    void write_QuadrantDao(int user_id,QuadrantDao item,QSqlDatabase database);

    void write_WorkflowDao(int user_id,WorkflowDao item,QSqlDatabase database);
    ...
    void read_WorkflowDao(int user_id,QSqlDatabase database,QStringList &ID,QStringList &ROOT,QStringList &BOOK,QStringList &NODE,QStringList &FL,QStringList &SI,QStringList &CH);

    //清空数据库
    void clean_QuadrantDao(int user_id,QSqlDatabase database);
    void clean_ScheduleDao(int user_id,QSqlDatabase database);
    void clean_WorkflowDao(int user_id,QSqlDatabase database);
