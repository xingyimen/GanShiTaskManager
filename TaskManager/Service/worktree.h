...
public:
    ...
    WorkTree(std::string name);   //构造函数（传入名称）（节点类型默认根、默认未读、默认索引），根节点
    WorkTree(WorkTree& parent,std::string name="未命名",int qttype=WorkTree::Unfinished);   //构造函数（传入节点名称、节点类型（索引/书））（默认孩子、默认未读）,孩子节点
    ~WorkTree();
    /*提供给Qt工作流界面显示的接口*/
    std::string return_progress();//返回目前在读的书名——提供给描述
    std::string return_root();//返回根节点名称——提供给标题
    /*树的删除*/
    bool delete_node(WorkTree*& root, int key);  //删除子节点
